...
type NameGenerator struct {
	familyNames   []string
	firstNames    []string
	nameIndexRand rand.Rand
}

func NewWithLen(len int) NameGenerator {
	return NameGenerator{
		familyNames:   strings.Split(familyNames, ""),
		firstNames:    strings.Split(firstNames, ""),
		nameIndexRand: *rand.New(rand.NewSource(100)),
	}
}

func (gen *NameGenerator) RandomFirstNames(count int) string {
	s := ""
	var firstNamesLen = len(gen.firstNames)
	for i := 0; i < count; i++ {
		var index = gen.nameIndexRand.Intn(firstNamesLen)
...
func (gen *NameGenerator) GetNames() Names {
	var ch = make(chan [4]string)
	go func() {
		for i := 0; i < len(gen.familyNames); i++ {
			var familyName = gen.familyNames[i]
			var father = familyName + gen.RandomFirstNames(2)
			famIndex := gen.nameIndexRand.Intn(len(gen.familyNames))
