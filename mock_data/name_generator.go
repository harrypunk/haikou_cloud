...
func NewWithSeed(seed int64) NameGenerator {
	return NameGenerator{
		...
		nameIndexRand: *rand.New(rand.NewSource(seed)),
	}
}
...
func (gen *NameGenerator) GetFamilyNames(familyNum int) Names {
	...
	go func() {
		for i := 0; i < familyNum; i++ {
			...
			famIndex := gen.nameIndexRand.Intn(famLen)
...
func (gen *NameGenerator) RandomNameList(count int) <-chan string {
